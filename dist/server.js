...
const DEV_ORIGIN_RE = /^http:\/\/(localhost|127\.0\.0\.1|192\.168\.\d{1,3}\.\d{1,3}|10\.\d{1,3}\.\d{1,3}\.\d{1,3}|172\.(1[6-9]|2\d|3[0-1])\.\d{1,3}\.\d{1,3})(:\d+)?$/;
async function main() {
    ensureDirs();
    await initMongo();
    const app = express();
    app.use(express.json({ limit: "2mb" }));
    const corsBase = { credentials: true };
    if (settings.env === "dev") {
        app.use(cors({
            ...corsBase,
            origin: (origin, cb) => {
                ...
                if (list.includes(origin) || DEV_ORIGIN_RE.test(origin))
                    cb(null, true);
                else
                    cb(null, false);
            },
