...
mysqlRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { host, user, password, db, port } = req.body as {
      host?: string;
      user?: string;
      password?: string;
      db?: string;
    ...
    if (!host || !user || password == null || !db) throw new HttpError(422, "validation_error");
...
mysqlRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!cfg) throw new HttpError(400, "mysql_not_configured");
    try {
