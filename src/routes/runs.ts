...
function readFileSafe(p: string | null | undefined): string {
  if (!p) return "";
  try {
...
runsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const r = oidToStr({ ...run }) as Record<string, unknown>;
    res.json({
      id: r.id,
      kind: r.kind,
      target: r.target,
      ...
      stdout: readFileSafe(r.stdout_path as string),
      stderr: readFileSafe(r.stderr_path as string),
    });
  })
);
