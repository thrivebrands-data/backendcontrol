...
export async function listScripts(userId: ObjectId): Promise<Record<string, unknown>[]> {
  const db = getDb();
  const items = await db
    ...
    .limit(1000)
    .toArray();
  return items.map((it) => oidToStr({ ...it }) as Record<string, unknown>);
}

export async function addScript(
  userId: ObjectId,
  input: { name: string; path: string; type: string }
  ...
  return oidToStr({ ...doc, _id: res.insertedId }) as Record<string, unknown>;
}

export async function deleteScript(userId: ObjectId, scriptId: string): Promise<void> {
  const db = getDb();
...
function launchInTerminal(command: string, cwd: string | undefined): void {
...
export async function runScript(
  ...
  | { run_id: string; status: string; launched_in_terminal: boolean }
  | { error: string }
> {
  const db = getDb();
  const oid = ensureObjectId(scriptId);
  ...
  const scriptPath = path.resolve(expandUserPath(script.path as string));
  ...
  if (useTerminal) {
    ...
    await db.collection("scripts").updateOne(
      ...
      { $set: { last_run_at: new Date(), last_status: "running" } }
    );
  ...
  const popenArgs =
    script.type === "python" ? [defaultPythonExe(), scriptPath] : [scriptPath];
  ...
  proc.on("close", async (rc) => {
    ...
    await db.collection("scripts").updateOne(
      { _id: oid, user_id: userId },
      { $set: { last_status: code === 0 ? "success" : "failed" } }
    );
  });
