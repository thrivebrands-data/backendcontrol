...
pipelineRouter.post("/run/python", asyncHandler(async (req, res) => {
    ...
    if (!pipelineScript)
        throw new HttpError(400, "pipeline_script_missing");
...
pipelineRouter.post("/run/bat", asyncHandler(async (req, res) => {
    const userId = await requireUserOid(req);
    const { client } = req.body;
    if (!client)
        throw new HttpError(422, "validation_error");
    ...
    const bat = ov?.bat_path || cfg.bat;
    if (!bat)
        throw new HttpError(400, "bat_missing");
