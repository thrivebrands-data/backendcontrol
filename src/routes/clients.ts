...
clientsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const overrides: Record<string, { bat_path: string | null | undefined; updated_at: unknown }> = {};
    for (const it of items) {
      overrides[String(it.client_key)] = {
        bat_path: (it.bat_path as string | null | undefined) ?? null,
...
clientsRouter.post(
  "/clients/overrides/bat",
  asyncHandler(async (req, res) => {
    const userId = await requireUserOid(req);
    const { client, bat_path: batPath } = req.body as { client?: string; bat_path?: string | null };
    if (!client) throw new HttpError(422, "validation_error");
    const clients = getClientsMap(settings.clientsConfigPath);
    if (!(client in clients)) throw new HttpError(404, "client_not_found");
    const db = getDb();
    const now = new Date();
    await db.collection("client_overrides").updateOne(
      { user_id: userId, client_key: client },
      {
