...
scriptsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    const userId = await requireUserOid(req);
    const { name, path: scriptPath, type } = req.body as { name?: string; path?: string; type?: string };
    if (!name || !scriptPath) throw new HttpError(422, "validation_error");
...
scriptsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if ("error" in result) throw new HttpError(404, "script_not_found");
    res.json(result);
  })
);
